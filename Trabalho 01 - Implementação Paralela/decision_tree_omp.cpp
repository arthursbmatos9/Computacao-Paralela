...
Tempos de execução:
	...
	-Paralela para 1 threads: 73315 ms        -> 73.31s
	-Paralela para 2 threads: 57767 ms        -> 57.76s
	-Paralela para 4 threads: 37708 ms		  -> 37.70s
	-Paralela para 8 threads: 56709 ms		  -> 56.70s
*/
#include <iostream>
#include <vector>
#include <string>
...
class DecisionTree {
	public:
		...
		int getSelectedAttribute(Table table) {
			...
			vector<double> gainRatios(numAttrs);
			...
			// Encontrar o máximo (sequencial, mas rápido)
			for(int i=0; i < numAttrs; i++) {
...
class AccuracyCalculator {
	public:
		static void calculate(DecisionTree& dt, Table& testTable) {
			...
			// PARALELIZAÇÃO: Predição em paralelo 
			vector<string> predictions(testTable.data.size());

			//PARALELIZAÇÃO: Popular o vetor de predições
			#pragma omp parallel for schedule(dynamic)
			...
			// Cálculo de acurácia (sequencial devido ao mapa)
			for(int i = 0; i < testTable.data.size(); i++) {
...
int main(int argc, const char * argv[]) {
	if(argc < 3 || argc > 6) {
		cout << "Uso: " << endl;
		cout << "  ./dt <arquivo.csv> <coluna_alvo> [resultado.csv] [proporção_treino] [num_threads]" << endl;
		cout << endl;
		cout << "Exemplos:" << endl;
		cout << "  ./dt cars.csv owner" << endl;
		cout << "  ./dt cars.csv owner resultado.csv" << endl;
		cout << "  ./dt cars.csv owner resultado.csv 0.7" << endl;
		cout << "  ./dt cars.csv owner resultado.csv 0.8 4" << endl;
		cout << endl;
	...
	cout << "Usando " << 8 << " threads OpenMP" << endl << endl;
	...
	// PARALELIZAÇÃO: Geração de predições em paralelo
	vector<vector<string>> results(testTable.data.size());
	
	// PARALELIZAÇÃO: Cada thread processa linhas independentes do conjunto de teste
	#pragma omp parallel for schedule(dynamic)
	for(int i = 0; i < testTable.data.size(); i++) {
	...
	// Escrita sequencial para preservar a ordem e evitar escritas concorrentes no arquivo.
